...
import { b as createAstro, c as createComponent, m as maybeRenderHead, d as addAttribute, r as renderTemplate, a as renderComponent, F as Fragment, A as AstroError, U as UnknownContentCollectionError, R as RenderUndefinedEntryError, u as unescapeHTML, h as escape, i as renderUniqueStylesheet, j as renderScriptElement, k as createHeadAndContent } from './astro/server_Bd8-x6y9.mjs';
...
import { V as VALID_INPUT_FORMATS } from './internal_Bjh2Ot_R.mjs';
import { u as unflatten } from './parse_EttCPxrw.mjs';

const $$Astro = createAstro("https://ericmaster.github.io");
const $$PostTeaser = createComponent(($$result, $$props, $$slots) => {
  const Astro2 = $$result.createAstro($$Astro, $$props, $$slots);
  Astro2.self = $$PostTeaser;
  const { post } = Astro2.props;
  return renderTemplate`${maybeRenderHead()}<li class="post-card"> <a${addAttribute("/posts/" + post.data.slug, "href")} class="flex flex-col md:flex-row items-center md:items-start gap-6 p-6 rounded-lg"> <div class="w-full lg:min-w-[300px] min-w-[200px]"> ${post.data.image && renderTemplate`<img${addAttribute(post.data.image.url, "src")}${addAttribute(post.data.image.alt, "alt")} class="w-full" loading="lazy" decoding="async" width="300" height="200">`} ${post.data.pubDate && renderTemplate`${renderComponent($$result, "Fragment", Fragment, {}, { "default": ($$result2) => renderTemplate` <small>${formatLocalizedDate(post.data.pubDate.toISOString())}</small> <br> ` })}`} </div> <div class="flex flex-col"> <h3> ${post.data.title} </h3> <span>${post.data.description}</span> ${post.data.tags && renderTemplate`<div class="tags mt-2"> ${post.data.tags.map((tag) => renderTemplate`<span class="tag">${tag}</span>`)} </div>`} </div> </a> </li>`;
}, "/app/src/components/PostTeaser.astro", void 0);
...
class ImmutableDataStore {
  ...
  static async fromModule() {
    try {
      const data = await import('./_astro_data-layer-content_9wE__548.mjs');
...
const __vite_import_meta_env__ = {"ASSETS_PREFIX": undefined, "BASE_URL": "/", "DEV": false, "MODE": "production", "PROD": true, "SITE": "https://ericmaster.github.io", "SSR": true};
...
async function updateImageReferencesInBody(html, fileName) {
  ...
  const { getImage } = await import('./_astro_assets_BWhvxIAb.mjs').then(n => n._);
...
export { $$PostTeaser as $, getCollection as g };
